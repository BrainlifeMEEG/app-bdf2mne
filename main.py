"""
Convert BioSemi BDF EEG files to MNE-Python raw format.

This app converts BioSemi .bdf files to MNE-compatible .fif format using the 
mne.io.read_raw_bdf function. It generates a report with channel information
and produces output raw data in the standard MNE format for downstream processing.

Input:
    - bdf: Path to BioSemi .bdf file
...
#
# This app converts BioSemi BDF EEG files to MNE raw format.
#
# Authors:
...
# == CREATE REPORT ==
report = mne.Report(title='BioSemi BDF to MNE Conversion Report')
...
# == CREATE PRODUCT JSON ==
product_items = []
add_info_to_product(product_items, f"BioSemi BDF file converted successfully")
